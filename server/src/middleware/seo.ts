...
const CRAWLER_PATTERN =
  /googlebot|bingbot|yandex|baiduspider|duckduckbot|slurp|facebookexternalhit|twitterbot|linkedinbot|whatsapp|telegrambot|kakaotalk|naver|daum/i;
...
function renderHomeContent(siteUrl: string): string {
...
function renderServiceContent(serviceSlug: string, siteUrl: string): string {
  const { services } = loadServices();
...
function renderCountryContent(
  ...
  const country = prices?.prices?.find(
    (item) => item.countryCode === countryCode.toUpperCase()
  );
...
function renderStaticContent(pageName: "about" | "privacy"): string {
  if (pageName === "about") {
    return `
      <section>
        <h2>서비스 목적</h2>
...
function renderTrendsContent(serviceSlug: string, siteUrl: string): string {
  const prices = loadPrices(serviceSlug);
...
function getSeoPayloadForPath(pathname: string, siteUrl: string): SeoPayload | null {
  if (pathname === "/") {
    return {
...
// 크롤러 감지 미들웨어
function seoMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  const isPreviewHost =
    typeof query.host === "string" && query.host.trim().length > 0;
